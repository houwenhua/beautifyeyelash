$(function(){
	...
	obj = {
		...
		//添加一行
		add:function(){
			 var content = "<iframe src='visitoradd.jsp' id='context' name='context' width='100%' height='99%' frameborder='0' scrolling='no'></iframe>";
		        $('#dd').dialog({
		            content:content,
		            title: '消费记录',
		            width: 600,
		...
		edit:function(){
			...
			if(rows.length == 1){
				var jq = top.jQuery;    
				var url = 'visitorupdate.jsp?id='+rows[0].id;
	...
	$("#datagrid").datagrid({
		width:700,
		fit:true,
		url:'visitor_paginationQuery.action',
		title:'消费记录',
		striped:true,
		rownumbers:true,
		fitColumns:true,
